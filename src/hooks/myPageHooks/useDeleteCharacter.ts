...
const useDeleteCharacter = () => {
   ...
   const {mutate: deleteCharacterMutation} = useMutation<void, unknown, {characterId: string, serverId: string}, unknown>(
     [QUERY_KEY.mypage], async ({characterId, serverId}) => await deleteCharacterFromUserAccount(characterId, serverId), {
        onSuccess: () => {
