...
const UserCharacters = (props: { data: Content[]; refresh: () => void }) => {
  ...
  const onClickDeleteButton = (e: React.MouseEvent) => {
    ...
    if (characterId && serverId && window.confirm('삭제하시겠습니까?')) {
      deleteCharacterFromUserAccount(characterId, serverId)
        .then((res) => {
  ...
  return <CharacterList data={props.data} onClick={onClick} deletable onClickDeleteButton={onClickDeleteButton} />;
};
