...
const CharacterSearchBoxChild = ({ direction,searchResult,searchHandler }: ICharacterSearchBoxChildProps, ref:ForwardedRef<any>) => {
  const [selectedMenu, setSelectedMenu] = useState<'최근 검색 기록' | '빠른 검색'>('최근 검색 기록');
  const { recentSearchedQuery, handleRemoveRecentSearchedQuery , handleAddRecentSearchedQuery } = useRecentSearchedQuery();
            ...
            <S.SearchOptionBody>
              {selectedMenu === '최근 검색 기록'
                ...
                recentSearchedQuery.map((data, index) => {
                  return (
                    <RecentSearchedItemButton {...data}
                      removeHandler={handleRemoveRecentSearchedQuery} mouseDownHandler={searchCallback} />)})
                : searchResult.map((item, index) => {
                  return (
                   <CharacterFastSearchItemButton {...item} mouseDownHandler={searchCallback}/>
                  );
                })}
              {isEmptyRecentSearchedList && selectedMenu === '최근 검색 기록' && (
                <NoData  content={'최근 검색 기록이 없습니다.'} />
              )}
              {isEmptySearchResult && selectedMenu === '빠른 검색' && (
                <NoData  content={'데이터가 존재하지 않습니다.'} />
              )}
            </S.SearchOptionBody>
