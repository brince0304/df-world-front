...
const SearchOptionTitleWrapper = styled(Box)`
  ...
  font-weight: bold;
  padding: 8px 10px;
  background-color: ${(props: { isselected: string }) =>
    props.isselected === 'true' ? 'cornflowerblue' : 'lightgray'};
...
interface SearchProps {
  ...
  autoCompleteHandler?: (url: string, setData: (object: SearchOption[]) => void) => void;
}
...
const CustomSearchBox = (props: SearchProps) => {
  const [open, setOpen] = useState(false);
  const [selectedFilter, setSelectedFilter] = useState<{
    label: string;
    value: string;
  }>({ label: props.filterOptions[0].label, value: props.filterOptions[0].value });
  ...
  useEffect(() => {
    if (searchValue === '') {
      ...
      if (props.autoCompleteUrl) {
        const url = props.autoCompleteUrl
          .replace('{searchValue}', searchValue)
          .replace('{selectValue}', selectedFilter.value);
        props.autoCompleteHandler?.(url, setAutoCompleteData);
      }
    }
  }, [searchValue, selectedFilter.value, isSelected, props]);
  ...
  return (
    <Box sx={containerStyle} ref={formRef}>
      <Paper component="form" sx={searchFilterWrapperStyle} onSubmit={handleSearch}>
          ...
          <SearchOptionContainer direction={props.direction}>
            <SearchOptionTitle>
              <SearchOptionTitleWrapper
                data-id={0}
                isselected={(isSelected === 0).toString()}
              ...
              <SearchOptionTitleWrapper
                data-id={1}
                isselected={(isSelected === 1).toString()}
                onMouseDown={handleChangeIsSelected}
              >
                빠른 검색
              </SearchOptionTitleWrapper>
            ...
            <SearchOptionBody>
              {isSelected === 0
                ? props.searchHistory?.length !== 0 &&
                  props.searchHistory?.map((data, index: number) => {
                    return (
                      <Button key={index} sx={{ padding: 0 }}>
                        ...
                        >
                          <BoldNameWrapper>{data.title}</BoldNameWrapper>
                          <ContentWrapper>{data.content}</ContentWrapper>
                          <ContentWrapper>{data.footer}</ContentWrapper>
                          {data.optionValue1 && <ContentWrapper>{data.optionValue1}</ContentWrapper>}
                        </HistoryOptionCell>
                        <LatestRemoveButtonWrapper data-id={data.id} onMouseDown={props.removeSearchHistory}>
                ...
                : autoCompleteData.map((item: SearchOption, index: number) => {
                    return (
                      <ListItemButton key={index} sx={{ padding: 0 }}>
                        ...
                        >
                          <BoldNameWrapper>{item.title}</BoldNameWrapper>
                          <ContentWrapper>{item.content}</ContentWrapper>
                          <ContentWrapper>{item.footer}</ContentWrapper>
                          {item.optionValue1 && <ContentWrapper>{item.optionValue1}</ContentWrapper>}
                        </OptionCell>
                      </ListItemButton>
                    );
                  })}
              {props.searchHistory?.length === 0 && isSelected === 0 && (
                <OptionRow>
                  <NoDataWrapper>검색 기록이 없습니다.</NoDataWrapper>
                </OptionRow>
              )}
              {autoCompleteData?.length === 0 && isSelected === 1 && (
                <OptionRow>
